...
def service_category(db, services_department):
    return Category.objects.create(
        ...
        description="Servicios de asesoramiento personalizado",
...
def test_services_catalog_shows_only_services_categories(client, services_department, service_category):
    """Test that services catalog shows only categories from Services department"""
...
@pytest.mark.django_db
def test_services_catalog_ignores_department_param(client, service):
    """Test that department parameter is ignored"""
    url = reverse("services:services_catalog") + "?department=999"
    response = client.get(url)

    assert response.status_code == 200
    # Department parameter should be ignored, service should still appear
    assert service in list(response.context['services'])
